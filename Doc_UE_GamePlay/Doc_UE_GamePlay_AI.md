...
- 智能行为：
  - **环境感知**：数据源的获取、传递
    - 具体示例：BT中的Blackboard、ST中的Context Property
  - **决策/方案**：根据已有信息做出行为选择
    - 常见概念：
      - FSM（有限状态机）：定义有限个状态 并 在其之间过渡切换的模型
      - BT（行为树）：基于根节点+控制节点+执行节点组成的树状结构，从根节点开始有序执行各节点
      - Utility（效用）：根据不同因素计算每个行为的效用值，选择效用值最该的行为执行
      - GOAP（Goal-Oriented Action Planning，目标导向行动规划）：基于 AI 规划技术，允许 AI 代理根据当前状态和目标 来动态生成行动序列，以最优方式实现其目标
      - HTN（Hierarchical Task Network，分层任务网络规划）：通过将复杂任务分解为更小、更易处理的子任务，从而逐步生成可执行的行动序列
    - 具体示例：BehaviorTree、StateTree、[Hierarchical Task Network Planning AI](https://maksmaisak.github.io/htn/front.html)
  - **行为/能力**：表现或逻辑处理
    - 具体示例：GameplayAbility、Montage、`UBTTaskNode`、`FStateTreeTaskBase`
- 环境准备：
  - 导航系统
    - 具体示例：`UNavigationSystemV1`
  - 能力组件
    - 具体示例：`UAIPerceptionComponent`、`UPathFollowingComponent`、`UBehaviorTreeComponent`、`UBlackboardComponent`
...
- [UE4 Navmesh寻路（一）Recast基础 - 知乎](https://zhuanlan.zhihu.com/p/74537236)
- [UE4 Navmesh寻路（二）Detour寻路 - 知乎](https://zhuanlan.zhihu.com/p/78873379)
- [Navigation与AI寻路 - 知乎](https://zhuanlan.zhihu.com/p/19965374177)
...
/*
 * 本质是对官方 UBTTask_FindAndUseGameplayBehaviorSmartObject 及 UAITask_UseGameplayBehaviorSmartObject 的融合改造：
 *	- 重定向了 移动逻辑的GoalLocation
 *
 * 功能特点：
 *	- 提供 移动逻辑的GoalLocation的选择（Entrance/Slot）
 *	- Task绑定了 GameplayBehavior生命周期 及 Slot状态，避免任何一方的遗漏操作
 *
 * 尚可优化：
 *	- 暴露更多移动相关参数到面板（类似参考 UBTTask_MoveTo）
 *
*/
 ...
 */
UCLASS()
class [PROJECTNAME]_API UBTTask_MoveAndUseSmartObject : public UBTTask_BlueprintBase
{
	GENERATED_BODY()
...
#include "BTTask_MoveAndUseSmartObject.h"
#include "AIController.h"
...
- [Unreal Engine AI：行为树、NavMesh、EQS、HTN、State Tree - 知乎](https://zhuanlan.zhihu.com/p/720166805)
